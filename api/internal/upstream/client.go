...
func isPolicyError(body []byte) bool {
	...
	if err := json.Unmarshal(body, &decoded); err == nil {
		joined := strings.ToLower(decoded.Error.Code + " " + decoded.Error.Message + " " + decoded.Error.Type)
		return strings.Contains(joined, "policy") ||
			strings.Contains(joined, "safety") ||
			strings.Contains(joined, "content")
	}
	lowerBody := strings.ToLower(string(body))
	return strings.Contains(lowerBody, "policy") ||
		strings.Contains(lowerBody, "safety") ||
